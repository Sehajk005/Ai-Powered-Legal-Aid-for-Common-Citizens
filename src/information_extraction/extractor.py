...
def extract_entities_with_llm(text):
    full_prompt = f"""You are an expert legal assistant. From the following document provided, you need to preform the following tasks:\
        ...
        2. 'clause_type': Catogerize the clause into a high level category from the following:[Termination, Payment, Liability, Confidentiality, Governing Law, Force Majeure, General, Other]\
        3. 'clause_text': Extract the text of the clause exactily as it appears in the document.
        4. 'summary_in_plain_english': Summarize the clause in plain english so that a non-legal person can understand the clause.
        5. 'potential_risks': access the potential risks for the citizen and provide a short summary of the potential risks.
        6. Analyze the entire document and return the result as a JSON format return a JSON array af all the clauses you can identify.\
...
def answer_user_questions(document_text, user_question):
    prompt = f"""You are a helpful Q&A chatbot Assistant. Your task is to answer the user question solely based on the provided document \
        text and not from any other source if the answer to the question is not found in the document text just simpley return with \
        ...
        Based on the the document above, answer the following question:\
